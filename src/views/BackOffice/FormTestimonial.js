...
import {Form,Input,Label,Button,MessageError,ButtonGroup} from "../../styles/FormTestimonial";
import {postTestimony,putTestimony,getOneTestimony} from "../../services/requests/form_testimonial";
...
function FormTestimonial({id,SetAnimation,data}){
    ...
    SetInputs({name:datos.data.name,image:datos.data.image,content:datos.data.content})
    }
    ...
    const {successMessage,success,errorMessage} = await postTestimony(inputs);
    if(success){
    ...
    inputs.content = ckeditor;
    const {successMessage,success,errorMessage} = await putTestimony(inputs,id);
    if(success){
    toast.success(successMessage);
    SetInputs({name:"",image:"",content:""});
    setTimeout(()=>{
